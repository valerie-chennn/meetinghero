...
- backstory：前情提要，用 \n 分行，结构化输出，共四层：
  ...
  - 第 2-4 行（角色立场）：每个 NPC 一行，格式"· [原名]（[在主场世界的适配头衔]）：[态度/立场]"。主场角色不需要括号。态度要体现该角色的性格特点，要有具体原因不能空泛。
...
3. 至少有一个词放在这个世界观里"不对劲"（现实入侵）
4. 读完脑子里蹦出的是"那咋办？"而不是"那是谁？"
5. 禁止：会议/议事/紧急/危机/讨论/决策/挑战——这些词不制造任何趣味
6. 禁止：廉价感叹词（竟然/居然/没想到）——冲击力来自事实本身
...
- 第 2-4 行（角色立场）：每个角色一行，格式"· [角色名]：[态度/立场]"。态度要体现该角色性格，要有具体原因不能空泛。
- 最后一行（钩子问题）：以"→"开头，一个开放式问题，指向会议要讨论的核心决策。
- 整体 ≤120 字（含换行符），用现代口语，禁止文言文
...

{
  ...
  "settingZh": "string（前情提要，≤80字，三层结构：大背景→冲突展开→钩子问题）",
