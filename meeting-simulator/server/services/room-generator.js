...
async function insertFeedItem(id, roomId, sortOrder) {
  return db.execute(
    'INSERT IGNORE INTO v2_feed_items (id, room_id, sort_order, is_visible) VALUES (?, ?, ?, 1)',
    [id, roomId, sortOrder]
  );
}
...
async function generateRoomsInBackground(count = 3) {
  ...
  const existingRows = await db.queryAll('SELECT news_title FROM v2_rooms');
  const existingTitles = existingRows.map(r => r.news_title);

  // 获取当前最小 sort_order，新房间排在种子后面（Feed 按 DESC 排序，sort_order 更小 = 排后面）
  const minSortRow = await db.queryOne('SELECT MIN(sort_order) AS minSort FROM v2_rooms');
  ...
  for (let i = 0; i < count; i++) {
    try {
      ...
      await insertFeedItem(feedItemId, roomId, nextSortOrder);
