...
- backstory：前情提要，用 \n 分行，结构化输出，共四层：
  - 第 1 行（大背景）：一句话交代这几个人在哪里、在一起干什么、出了什么事。只用角色原名，不加适配头衔（适配头衔在下面角色立场行里展示）。必须包含一个融合角色特色的项目名/活动名/事件名。用大白话写。
  - 第 2-4 行（角色立场）：每个 NPC 一行，格式"· [原名]（[适配头衔]）：[态度/立场]"。主场角色不需要括号。态度要体现角色性格，要有具体原因。
  - 最后一行（钩子问题）：以"→"开头，一个开放式问题，指向会议核心决策。
  - 整体 ≤120 字（含换行符），用现代口语，禁止文言文
  - 禁止介绍用户的身份、职位或名字
  - **因果连贯性（最重要）**：大背景里每个元素必须有因果关系，读者不需要猜"这是什么""为什么会这样"。禁止用模糊的代称（"西洋匣子""那东西"），必须说清楚具体是什么、为什么会出问题。
    坏例："西洋匣子一炸，两人全断"（什么匣子？为什么炸了人就断了？）
    好例："大观园办海棠诗灯会，爱玛带来的电灯泡炸了，园里一片漆黑"（什么活动→谁带了什么→出了什么事→什么后果，每步都连着）
- goal：1句话核心目标，用大白话写
