  ...
  // ──────────────────────────────────────────────
  {
    ...
    dialogue_script: JSON.stringify([
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "Hey auditor, first time in the group? What do you think about this situation?", hintZh: '嘿，审计员，第一次进群？你怎么看这件事？',
        // 审计员初入群，被问对卖马事件的第一印象
        options: [
          { label: '保持中立', example: "I just got here — I'd need more info before I take a side." },
          { label: '支持卖马', example: "If the project is over, maybe it makes sense to let him go." },
          { label: '反对卖马', example: "Selling a team member feels wrong, no matter the reason." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_b', hint: "Okay, as the auditor here — who do you think actually contributed more to this project?", hintZh: '好，作为审计员——你觉得谁对这个项目的贡献更大？',
        // 白龙马追问贡献归属，两边都有话说
        options: [
          { label: '挺白龙马', example: "Physical work is real work. The horse carried the team." },
          { label: '挺八戒', example: "Soft skills matter too — morale is part of the job." },
          { label: '各有贡献', example: "Both of you played a role. It's hard to compare directly." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "So — do you think it's fair to sell a team member after the project ends? What would you do?", hintZh: '那么——你觉得项目结束后把队友卖掉公平吗？你会怎么做？',
        // 八戒直接问公不公平，要审计员表态
        options: [
          { label: '明确反对', example: "No. You don't sell people. That's not how teams work." },
          { label: '看合同说话', example: "It depends on what was agreed before the project started." },
          { label: '建议和解', example: "Instead of selling, why not figure out what he needs next?" },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    dialogue_script: JSON.stringify([
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "Hey HR, you're supposed to onboard Thanos today. How's that going?", hintZh: '嘿HR，你今天负责给灭霸办入职的。进展怎么样？',
        // HR被问入职进展，灭霸已经搞出事了
        options: [
          { label: '进展不顺', example: "Honestly? He already tried to use the Gauntlet. We had to step in." },
          { label: '谨慎乐观', example: "It's early, but he seems to follow instructions when you're direct." },
          { label: '还在摸底', example: "I'm still getting a read on him. Give me a bit more time." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_b', hint: "You seem to have experience with ambitious newcomers. What is your advice for handling someone like Thanos?", hintZh: '您似乎对处理有野心的新人很有经验。处理灭霸这样的人，您有什么建议？',
        // 甄嬛问HR如何管理灭霸这类野心型新人
        options: [
          { label: '给小项目试水', example: "Start him on something small. Let's see how he handles limits." },
          { label: '建清晰边界', example: "Be very clear about what he can and cannot do here." },
          { label: '直接提离职', example: "If someone can't follow basic rules on day one, that's a red flag." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "So — talented but dangerous. As HR, how do you handle an employee like this?", hintZh: '所以——有才但危险。作为HR，你怎么处理这样的员工？',
        // 钢铁侠总结局面，要HR给出结论性建议
        options: [
          { label: '保留并管控', example: "Keep him, but take away the tools that let him cause damage." },
          { label: '设试用期', example: "Give it another month. If he breaks the rules again, we let him go." },
          { label: '建议辞退', example: "Talent doesn't justify the risk. I'd recommend parting ways." },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    dialogue_script: JSON.stringify([
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "You were there. Can you describe exactly what happened when the stage froze?", hintZh: '你当时在场。能描述一下舞台冻住时到底发生了什么吗？',
        ...
        options: [
          { label: '客观还原', example: "The music started, the stage froze within seconds. Everyone was surprised." },
          { label: '帮Elsa说话', example: "It was an accident. She didn't mean to freeze anything, I think." },
          { label: '突出损失', example: "The microphone, the chairs, the keynote — everything just stopped working." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_b', hint: "Elsa, the guests are cold. Is there anything you can do to help right now?", hintZh: 'Elsa，嘉宾们很冷。你现在能做点什么来帮忙吗？',
        // 助理转达需求，要Elsa具体做点什么
        options: [
          { label: '让她化冰', example: "Can you melt the ice on the stage? We really need the space back." },
          { label: '让她道歉', example: "Maybe start by saying sorry to the guests? That would help." },
          { label: '变废为宝', example: "Actually — can you make the ice look intentional? Like a theme?" },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "As the event assistant, what do you think — how do we prevent this from happening at next year's event?", hintZh: '作为活动助理，你觉得——我们怎么防止明年的活动再次发生这种事？',
        ...
        options: [
          { label: '加审批流程', example: "Any special abilities at the venue should need approval in advance." },
          { label: '纳入策划组', example: "Honestly, put Elsa on the planning team. Then at least it's controlled." },
          { label: '提前彩排', example: "We should do a full run-through with everyone before the event." },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    dialogue_script: JSON.stringify([
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "As the tech consultant here — can you explain how the AI sorting system actually works?", hintZh: '作为技术顾问——你能解释一下AI分院系统是怎么工作的吗？',
        ...
        options: [
          { label: '技术解释', example: "It analyzes personality data and matches patterns to each house's profile." },
          { label: '承认局限', example: "It's accurate on average, but it can miss nuance in individual cases." },
          { label: '为系统辩护', example: "The data is solid. 97% accuracy is hard to argue with." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_b', hint: "Draco, setting aside your own result — do you think AI should be used for decisions like this?", hintZh: '马尔福，先不说你自己的结果——你觉得AI应该用来做这类决策吗？',
        ...
        options: [
          { label: '支持使用', example: "If the data is good, I think AI can make better decisions than humans." },
          { label: '有条件支持', example: "It could help, but only as a suggestion — not the final word." },
          { label: '明确反对', example: "No. Decisions about who you are shouldn't be made by a machine." },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "Last question — what is one thing you think AI should never replace, no matter how accurate it gets?", hintZh: '最后一个问题——你觉得有什么是AI无论多准确都不应该替代的？',
        // 最后一问，要用户说出AI的边界在哪里
        options: [
          { label: '人际关系', example: "Real human connection — that's something AI just can't replicate." },
          { label: '情感判断', example: "Understanding how someone actually feels. Data can't capture that." },
          { label: '道德决策', example: "Choosing what's right when the answer isn't clear. That needs a human." },
        ],
      },
  ...
  // ──────────────────────────────────────────────
  {
    ...
    dialogue_script: JSON.stringify([
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "Gandalf, as the reporter here — can you walk me through what happened with the chair? Step by step?", hintZh: '甘道夫，作为记者——你能跟我说一下转椅那件事的经过吗？一步一步地说。',
        ...
        options: [
          { label: '追问细节', example: "Let's be specific — did you press the button, or did the chair just move on its own?" },
          { label: '帮他圆场', example: "I'm sure it was the venue's setup. These things happen at live events." },
          { label: '引出法杖', example: "Some witnesses mentioned your staff started glowing. Can you speak to that?" },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_b', hint: "Gollum, your performance got a lot of attention tonight. How do you think you did?", hintZh: '咕噜，你今晚的表演引起了很多关注。你觉得自己发挥得怎么样？',
        ...
        options: [
          { label: '真心肯定', example: "Honestly, it was memorable. Nobody else performed like that tonight." },
          { label: '委婉批评', example: "You definitely stood out. The judges' score didn't quite reflect that though." },
          { label: '问他感受', example: "The audience had a big reaction. What was going through your mind on stage?" },
        ],
      },
      ...
      { type: 'user_cue', speaker: 'npc_a', hint: "Final question for both of you — what do you think makes a truly great performance?", hintZh: '最后一个问题问你们两位——你们觉得什么是真正出色的表演？',
        // 收尾问题，记者可以表达自己的观点立场
        options: [
          { label: '技巧为先', example: "I think skill and preparation matter most. You can't fake that on stage." },
          { label: '情感为先', example: "For me, it's about emotion. If the audience feels something, that's a great performance." },
          { label: '两者兼顾', example: "You need both — the technique to deliver it, and the heart to mean it." },
        ],
      },
