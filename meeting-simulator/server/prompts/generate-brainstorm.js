...
- topic 和 topicZh 是中英文对照的会议标题，必须遵守以下规则：
  - 必须包含一个具体事件（动词+物品）+ 一个反差物（不该出现在这个世界观里的东西）
  - 禁止自造新概念/新名词，用户要能一眼看懂每个词
  - 禁止纯抽象描述（"失衡""议事""定策"不是事件）
  - topicZh 要标题党，≤20 字
  - 坏：关于"等价之门失衡"的紧急议事 → "等价之门"是自造概念，"议事"没有事件
  - 好：炼金实验室爆炸：三吨黄金变成了猫砂 → 每个词用户都懂，猫砂制造荒诞感
...
- backstory：恰好 2 句，用 \\n 分隔，每句必须有独立信息量，禁止废话铺垫
  ...
  - 第 2 句：你在这里的角色是什么？你手里有什么别人没有的筹码？
    好例：阎王爷点你来，因为你手里有三百年的人间观察记录，其他人拿不到。
    坏例：你必须在场，因为最终认证名录由你签字。（"你必须在场"太僵硬）
    禁止以"你必须在场"或类似句式开头。
  - 禁止介绍用户的身份、职位或名字
- goal：1句话核心目标（贴合世界观）
...
title 和 settingZh 必须让用户一看就知道发生了什么事，同时觉得离谱。

### title 必须包含：
1. 一个具体事件（动词+物品，不是抽象名词）——让用户一看就知道发生了什么
2. 一个反差物——一个不该出现在这个场景/世界观里的东西，这是荒诞感的来源

### title 禁止：
- 自造新概念/新名词（用户要能一眼看懂标题里的每个词）
- 纯抽象描述（"失衡""定策""议事"——这些不是事件）
- 没有事件的标题（"XX 紧急会议"不够，必须说出什么事）

### settingZh 补充细节：
- 在 title 基础上补充具体细节（数字/物品/地点/后果）
- 语言风格匹配主场世界（古典主场可用文言，当代主场用口语，动漫主场用二次元语气）

### 好/坏对比（理解边界用，不要模仿句式）：
坏：关于"等价之门失衡"的紧急议事
→ 坏在哪："等价之门"是自造概念，用户不知道什么门；"失衡"是抽象状态；"议事"没有事件
好：炼金实验室爆炸：三吨黄金变成了猫砂
→ 好在哪：每个词都是用户已知的，事件具体（爆炸），猫砂不该出现在实验室里制造了荒诞感

### 质量标准
- title：中文，≤20 字，必须包含具体事件+反差物
- settingZh：一句话，≤50 个中文字，补充 title 没写的细节
...
{
  "title": "string（中文主题标题，≤20字，必须包含具体事件+反差物）",
  "settingZh": "string（一句话场景设定，≤50个中文字，补充标题没写的细节）",
