...
## 场景戏剧化（settingZh 和整体世界观设定）
settingZh 必须包含至少一个夸张/荒诞的具体细节（数字、物品、后果），让用户一读就觉得"这也行？"
规则：
- 涉及损失/危机时，必须量化（多少钱/多少人/多少时间）
- 物品/道具必须具体到可以画面化（不是"货物"，而是某种特定的东西）
- 如果场景描述可以直接用在正经新闻里，就说明不够荒诞，必须重写
- 荒诞感来自"严肃角色 + 离谱处境"的反差，不是无厘头
...
## 主题名和描述风格
主题名（title）和场景设定（settingZh）的语言风格必须匹配主场世界，不能统一用文言古风：
- 中国古典/中国神话主场：可用文言风格（"幽都生死清册"）
- 当代名人主场：用现代口语风格（"硅谷紧急董事会：谁动了我的服务器"）
- 动漫主场：用二次元风格（"高专临时作战会议！咒灵大暴走"）
- 影视主场：用电影感风格（"黑珍珠号甲板密谈：藏宝图撕成了三份"）
- 西方文学/历史主场：用戏剧感现代中文（"凯撒的圆桌：谁在背后捅刀"）
- 神话主场（希腊/北欧）：用史诗感但不文言（"奥林匹斯紧急峰会：宙斯的王座在动摇"）

settingZh 也同样规则——用主场世界的语气写，不用文言。
