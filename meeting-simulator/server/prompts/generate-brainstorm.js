...
function buildCharacterStyleTriple(char, cultureType, isMainWorldChar, isMixed) {
  ...
  // 优先用英文名，无英文名的角色（中国古典等）兜底用中文名
  const displayName = char.nameEn || char.name;
  return `- ${displayName}（来自 ${char.worldLabel}）${mixedNote}
    ${speakingStyle}
    ${anchorStyle}
    ${reactionStyle}
    ${forbiddenStyle}`;
...
用户身份：${theme.userRole}
${theme.characters ? `角色适配头衔：\n${theme.characters.map(c => `- ${c.nameEn || c.name}：${c.adaptedTitle}`).join('\n')}` : ''}
` : '';
...
- 参与角色：${characters.map(c => c.nameEn || c.name).join('、')}
...
## 用户称呼（脑洞模式）
脑洞模式下，NPC 对话中称呼用户时使用 userRole.title（即本次生成的角色头衔），不使用花名。
例如：如果用户的角色头衔是"幽都判籍冥君"，NPC 叫"判籍冥君"或"冥君"，不叫花名。
dialogue 中用户的称呼统一用角色头衔的简称。

## 段落叙事弧线：自然引出用户发言（最高优先级规则）
每段 NPC 对话必须自然地把话题引向用户，禁止 NPC 之间聊完后无过渡直接出现 keyNode。
用户的角色头衔简称在每段 cue 语句中必须出现至少 1 次（用 userRole.title 的简称，不用花名）。
...
- status/statusZh 用世界观语言描述当前局势，禁止现代企业词汇

## userRole（角色卡，全部中文）
- title：用户在本场的角色头衔，5-10 字。必须跟 backstory 里的事件直接相关——这个头衔要让用户一看就知道"为什么这件事需要我来拍板"。如事件是雷车赔偿→"奥林波斯赔偿裁判"，事件是剧本争议→"制片总监"。禁止跟事件无关的通用头衔。NPC 对话中用此头衔称呼用户。
- backstory：前情提要，用 \n 分行，结构化输出，共四层：
  - 第 1 行（大背景）：一句话交代这几个人在哪里、在一起干什么、出了什么事。必须包含一个融合角色特色的项目名/活动名/事件名（如汉尼拔+唐僧拍悬疑片→片名要融合两人特色产生荒诞感）。用大白话写。
  - 第 2-4 行（角色立场）：每个 NPC 一行，格式"· [英文名]（[在主场世界的适配头衔]）：[态度/立场]"。主场角色不需要括号。态度要体现该角色的性格特点，要有具体原因不能空泛。角色名必须用英文名（如 Tony Stark、Hermione、Loki）；无英文名的中国古典角色用其中文名。
  - 最后一行（钩子问题）：以"→"开头，一个开放式问题，指向会议要讨论的核心决策。
  - 整体 ≤120 字（含换行符），用现代口语，禁止文言文
  - 禁止介绍用户的身份、职位或名字
  - **因果连贯性（最重要）**：大背景里提到的地点、事件、物品之间必须有明确的因果关系。每个元素都要能回答"为什么会出现在这里"。坏例："精神病院里商量慈悲晚宴录音怎么收口"（精神病院和慈悲晚宴什么关系？录音为什么在这里？逻辑断裂）。好例："三人合伙在横店拍悬疑片《吃斋的羔羊》，首映票房扑街"（谁→在哪→干什么→出了什么事，每一步都连着）
- goal：1句话核心目标，用大白话写
...

{
  ...
  "memo": [{"text":""}],
  "roles": [{"name":"（英文名，如 Tony Stark；无英文名的中国古典角色用中文名）","title":"","type":"leader|collaborator|challenger","avatar":"","briefNote":"","stance":"ally|neutral|pressure"}],
  "dialogue": [
    {"speaker":"英文名（与 roles[].name 保持一致）","text":"","textZh":"","isKeyNode":false},
...
- 主场景世界：${mainWorldLabel}
- 参与角色：
${characters.map(c => `  - ${c.nameEn || c.name}（来自 ${c.worldLabel}）：${c.persona}`).join('\n')}
...
- 第 2-4 行（角色立场）：每个角色一行，格式"· [英文名]（[适配头衔]）：[态度/立场]"（主场景角色不需要括号）。角色名必须用英文名（如 Tony Stark、Hermione、Loki）；无英文名的中国古典角色用其中文名。态度要体现该角色性格，要有具体原因不能空泛。
- 最后一行（钩子问题）：以"→"开头，一个开放式问题，指向会议要讨论的核心决策。
- 整体 ≤120 字（含换行符），用现代口语，禁止文言文
