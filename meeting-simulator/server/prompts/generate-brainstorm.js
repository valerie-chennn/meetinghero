...
### 语言风格
- 脑洞模式鼓励"现实入侵"：现代词汇（CTO、外卖、预算、快递）出现在古代/幻想世界里是有趣的，不需要替换
...
- 点将局：会议地点和设定贴合主场景，角色都在自己的世界里，不需要适配头衔
- 乱炖局：主场景是 ${mainWorldLabel}，其他角色适配进来（只改头衔，保留性格）。适配头衔用大白话，让用户一看就懂这个角色在这个世界干什么，禁止生造晦涩古风词
...
- topic 和 topicZh 是中英文对照的会议标题，必须遵守以下规则：
  ...
  ### topicZh（中文标题）——现实入侵式
  核心武器：把现代/现实世界的词汇（合同、外卖、预算、快递、社保）放进古代/幻想世界，荒诞感来自词汇的错位。
...
- backstory：前情提要，用 \n 分行，共三部分：
  ...
  第 1 行 = 大背景。一句大白话，说清谁在哪干什么、出了什么事。只写角色原名，**绝对禁止在这行出现括号和适配头衔**。每句话主语谓语宾语完整，读起来通顺。所有地名、事件、概念必须是中国大众能理解的——禁止用只有原著粉才懂的冷门设定（如"海伯里""阿斯加德彩虹桥议事厅"），用大白话描述地点（如"爱玛家的庄园""北欧神殿"）。

  第 2-4 行 = 角色立场。每个 NPC 一行，格式严格为：
  · 点将局（同世界）：直接写"· 角色原名：主张"，不加括号和头衔——热门 IP 角色身份已定，不需要解释
  · 乱炖局（跨世界）：非主场角色写"· 角色原名（适配头衔）：主张"，主场角色不加括号但保留"· "前缀
  **必须写"对这件事要怎么办"，禁止写通用性格描述。** 坏："他不信空话，只认数据"（这是性格，不是主张）。好："他要先查账，算清亏了多少再决定"（这是对事件的具体行动主张）。
...
核心武器：**现实入侵式**——把现代/现实世界的词汇（合同、外卖、预算、快递、社保）放进古代/幻想世界，荒诞感来自词汇的错位。
- 好：悟空要签合同 / 我们被外卖平台告了 / 空城计被录下来了 / 园子钱花完了
- 坏：天庭紧急会议 / 荣国府出大事了（没有具体事件，没有错位感）
...
- 第 2-4 行（角色立场）：每个角色一行。点将局（同世界）格式"· [原名]：[态度/立场]"，不加括号头衔；乱炖局（跨世界）非主场角色格式"· [原名]（[适配头衔]）：[态度/立场]"，主场角色不加括号。态度要体现该角色性格，要有具体原因不能空泛。
  ...
  "characters": [
    {
      "id": "string（角色ID）",
      "name": "string（角色名）",
      "adaptedTitle": "string（乱炖局：非主场景角色给出适配头衔，主场景角色留空；点将局：全部留空）",
